...
int dwt_writetxdata(uint16 txFrameLength, uint8 *txFrameBytes, uint16 txBufferOffset)
{
    ...
    // Write the data to the IC TX buffer, (-2 bytes for auto generated CRC)
		    if (txBufferOffset == 0)                                         // for index of 0, no sub-index is required
    {
        header[cnt++] = 0x80 | TX_BUFFER_ID ;               // bit-7 is WRITE operation, bit-6 zero=NO sub-addressing, bits 5-0 is reg file id
    }
    else
    {
        header[cnt++] = 0xC0 | TX_BUFFER_ID ;               // bit-7 is WRITE operation, bit-6 one=sub-address follows, bits 5-0 is reg file id

        if (txBufferOffset <= 127)                                   // for non-zero index < 127, just a single sub-index byte is required
        {
            header[cnt++] = (uint8)txBufferOffset ;                  // bit-7 zero means no extension, bits 6-0 is index.
        }
        else
        {
            header[cnt++] = 0x80 | (uint8)(txBufferOffset) ;           // bit-7 one means extended index, bits 6-0 is low seven bits of index.
            header[cnt++] =  (uint8) (txBufferOffset >> 7) ;           // 8-bit value = high eight bits of index.
        }
    }

    // write it to the SPI
    return writetospi_dma(cnt,header,txFrameLength-2,txFrameBytes);
} // end dwt_writetxdata()
...
//    if (rxBufferOffset == 0) // For index of 0, no sub-index is required
//    {
//        header[cnt++] = (uint8) RX_BUFFER_ID ; // Bit-7 zero is READ operation, bit-6 zero=NO sub-addressing, bits 5-0 is reg file id
//    }
//    else
//    {
//#ifdef DWT_API_ERROR_CHECK
...
//        }
//#endif
//        header[cnt++] = (uint8)(0x40 | RX_BUFFER_ID) ; // Bit-7 zero is READ operation, bit-6 one=sub-address follows, bits 5-0 is reg file id

//        if (rxBufferOffset <= 127) // For non-zero index < 127, just a single sub-index byte is required
//        {
//            header[cnt++] = (uint8) rxBufferOffset ; // Bit-7 zero means no extension, bits 6-0 is index.
//        }
//        else
//        {
//            header[cnt++] = 0x80 | (uint8)(rxBufferOffset) ; // Bit-7 one means extended index, bits 6-0 is low seven bits of index.
//            header[cnt++] =  (uint8) (rxBufferOffset >> 7) ; // 8-bit value = high eight bits of index.
//        }
//    }

//    // Do the read from the SPI
//     readfromspi_dma(cnt, header, length, buffer);  // result is stored in the buffer
//}
void dwt_readrxdata(uint8 *buffer, uint16 length, uint16 rxBufferOffset)
{
